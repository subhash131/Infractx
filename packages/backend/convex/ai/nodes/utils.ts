...
export const validateFrame = async (state: WorkflowStateType) => {
    ...
    if (state.frameId) {
      ...
      const frame = await state.convexState.runQuery(api.design.layers.getLayerById, {
      ...
      if (!frame || frame.type !== "FRAME") {
        ...
        frameId = await state.convexState.runMutation(api.design.layers.createObject, {
          name: "New Frame",
          type: "FRAME",
          pageId: state.pageId,
          width: 800,
...
export const insertLayer = async (layer: Doc<"layers">,state: WorkflowStateType) => {
    const layerId = await state.convexState.runMutation(api.design.layers.createObject, {
        ...layer,
    });
    return layerId
}